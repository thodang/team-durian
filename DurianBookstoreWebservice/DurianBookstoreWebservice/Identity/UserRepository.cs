    ...
    public class UserRepository : IUserRepository
    {
        ...
        public async Task UpdateUserAsync(User user)
        {
            ...
            try
            {
                ...
                var updateUser = Builders<User>.Update.Set(x => x.web_sessions, user.web_sessions);
                var bookUpdateResult = await users.UpdateOneAsync(userUdateFilter, updateUser);
            }
            catch (Exception e)
            {
                throw new Exception(e.Message);
        ...
        {
            var expiredSessions =
                user.web_sessions.Where(x => DateTime.Compare(x.ExpirationDateTime, DateTime.UtcNow) < 0);

            foreach (var session in expiredSessions.ToArray())
            {
                user.web_sessions.Remove(session);
            }
